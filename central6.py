    ...
    if request.method == 'POST':
     try:
        ...
        if(len(missing_attr)==0): #i.e all mandatory fields are present in user input request body
                ...
                entry={"cn":data['username'],"sn":data['lastname'],"givenName":data['firstname'],"displayName":data['displayName'],"o":data['organization'],"c":data["country"],"objectClass":"inetOrgPerson","mobile":'+'+data['mCode']+data['phone'],"mail":data['email'],"userPassword":data['password'],"uid":data["empId"],"departmentNumber":data["incidentId"]}
